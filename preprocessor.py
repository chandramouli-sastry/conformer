...
PreprocessorConfig = namedtuple(
    'PreprocessorConfig',
    field_names=[
        'sample_rate', 'frame_size_ms', 'frame_step_ms', 'compute_energy',
        'window_fn', 'output_log_floor', 'pad_end', 'preemph',
        'preemph_htk_flavor', 'noise_scale', 'num_bins', 'lower_edge_hertz',
        'upper_edge_hertz', 'fft_overdrive', 'output_floor'
    ])


def _hertz_to_mel(frequencies_hertz):
    """Convert hertz to mel."""
    log_fn = math.log if type(frequencies_hertz) in [type(0.0),
                                                     type(0)] else torch.log
...
def linear_to_mel_weight_matrix(num_mel_bins: int = 20,
    ...
    linear_frequencies = torch.linspace(0.0,
                                        nyquist_hertz,
    ...
    edges = torch.linspace(_hertz_to_mel(lower_edge_hertz),
    ...
    lower_slopes = (spectrogram_bins_mel - lower_edge_mel) / (center_mel -
                                                              lower_edge_mel)
    upper_slopes = (upper_edge_mel - spectrogram_bins_mel) / (upper_edge_mel -
                                                              center_mel)

    # Intersect the line segments with each other and zero.
    mel_weights_matrix = torch.minimum(lower_slopes,
                                       upper_slopes).clamp(min=0.0, max=None)
...
class SpectrogramFrontend(nn.Module):
    ...
    def __init__(self,
        ...
        self._frame_size = int(round(p.sample_rate * p.frame_size_ms /
                                     1000.0)) + 1  # +1 for the preemph
        ...
        if p.window_fn is None:
            ...
            def _hanning_window(frame_size, dtype):
                ...
                if frame_size % 2 == 0:
                    ...
                    return torch.hann_window(window_length=frame_size,
                                             periodic=True,
                                             dtype=dtype,
                                             device=device)
                else:
            ...
            def f(frame_size, dtype):
                return _hanning_greco(self._frame_size - 1,
                                      frame_size,
                                      dtype,
                                      device=device)
            ...
            raise ValueError('Illegal value %r for window_fn param' %
                             p.window_fn)
    ...
    def fprop_paddings(self, input_paddings):
        ...
        if p.pad_end:
            num_extends = _pad_end_length(input_paddings.shape[1],
                                          self._frame_step, self._frame_size)
            input_paddings = F.pad(input_paddings, (0, num_extends), value=1.0)
        x = input_paddings.unfold(dimension=1,
    ...
    def forward(self, inputs, input_paddings):
        ...
        framed_signal = frame(pcm_audio_chunk,
                              self._frame_size,
                              self._frame_step,
                              pad_end=p.pad_end)
...
class MelFilterbankFrontend(nn.Module):
    ...
    def __init__(self,
        ...
        self.stft = SpectrogramFrontend(p,

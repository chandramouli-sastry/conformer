...
class Subsample(nn.Module):
    ...
    def __init__(self, encoder_dim: int = 0, input_dropout_rate: float = 0.0):
        ...
        self.conv1 = Conv2dSubsampling(input_channels=1,
...
class Conv2dSubsampling(nn.Module):
    ...
    def __init__(self,
        ...
        self.unfold_logic = unfold_logic
    ...
    def get_same_padding(self, input_shape):
        ...
        if in_height % stride_height == 0:
            ...
            pad_along_height = max(filter_height - (in_height % stride_height),
                                   0)
        ...
        padded_paddings = torch.cat([
            ...
            torch.zeros(size=(paddings.shape[0], 1, pad_len),
                        device=paddings.device)
                                    ...
                                    dim=2)
        out_padding = F.conv1d(input=padded_paddings,
...
class FeedForwardModule(nn.Module):
    ...
    def __init__(self, config: ConformerConfig):
        ...
        self.linear1 = nn.LazyLinear(out_features=config.encoder_dim *
                                     config.feed_forward_expansion_factor,
                                     bias=True)
        self.dropout1 = nn.Dropout(p=config.feed_forward_dropout_rate)
        self.linear2 = nn.LazyLinear(out_features=config.encoder_dim,
                                     bias=True)
...
class AddPositionalEmbedding(nn.Module):
    ...
    def __init__(self,
        ...
        log_timescale_increment = math.log(
            ...
            float(self.min_timescale)) / (num_timescales - 1)
    ...
    def forward(self, seq_length):
        position = torch.arange(end=seq_length,
                                dtype=torch.float32,
        ...
        if self.embedding_dim % 2:
            signal = torch.cat(
                [signal,
                 torch.zeros(signal.shape[0], signal.shape[1], 1)],
                dim=2)
        return signal
...
class MHSAwithQS(nn.MultiheadAttention):
    ...
    def __init__(self, config: ConformerConfig):
        super().__init__(embed_dim=config.encoder_dim,
            ...
            if not why_not_fast_path:
                ...
                query = self.qs(
                    query.view(query.shape[0], query.shape[1], self.num_heads,
                               self.embed_dim //
                               self.num_heads)).view(*query.shape)
                in_proj_bias = self.in_proj_bias + 0
                in_proj_bias[:self.embed_dim] = self.qs(
                    self.in_proj_bias[:self.embed_dim].view(
                        self.num_heads,
                        self.embed_dim // self.num_heads)).view(-1)
                return torch._native_multi_head_attention(
                    query, key, value, self.embed_dim, self.num_heads,
                    self.in_proj_weight, in_proj_bias, self.out_proj.weight,
                    self.out_proj.bias, key_padding_mask
...
class CustomBatchNorm1d(nn.BatchNorm1d):
    ...
    def __init__(self, config: ConformerConfig):
        super().__init__(num_features=config.encoder_dim,
